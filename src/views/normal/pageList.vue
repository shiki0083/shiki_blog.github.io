  ...
  <div>
    <transition name="fade">
      <div v-if="page_list.length">
        <one-page v-for="(page, i) in page_list" :key="i" :page="page" />
        <div class="pagination" style="margin-bottom: 20px;">
          <Pagination v-if="total > pageSize" :total="total" @on-change="pageChange" />
        </div>
      </div>
    </transition>
  </div>
...
export default {
  ...
  methods: {
    getPageList() {
      this.Common.axios("/api/v1/videoList").then(res => {

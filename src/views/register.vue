    ...
    <div class="login-div">
      ...
      <Form ref="registerForm" :model="formData" :rules="rule" class="login-form">
        <FormItem prop="userName">
          ...
          <Input type="text" v-model="formData.userName" placeholder="用户名" transfer />
        </FormItem>
        ...
        <FormItem prop="passwordConfirm">
          <Input type="password" style="display:none" />
          <Input type="password" v-model="formData.passwordConfirm" placeholder="再次输入密码" />
        </FormItem>
        <FormItem>
  ...
  data() {
    ...
    return {
      ...
      rule: {
        ...
        passwordConfirm: [{ validator: validatorRepass, trigger: "blur" }]
      }
    };
  },
  methods: {
    ...
    register(name) {
      this.$refs[name].validate(valid => {
        if (valid) {
          ...
          this.Common.axios("/api/v1/user/register", this.formData).then(
            res => {
              // 注册成功直接登录
              const {
                data: {
              ...
              //   } else {
              //     this.$router.push({
              //       path: `/user/${this.formData.userName}`
              //     });
              //   }
              // }, 2000);
            }
